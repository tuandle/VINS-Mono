...
FeatureTracker trackerStereo[2];
double first_image_time, first_left_time, first_right_time;
int pub_count = 1;
bool first_image_flag = true;
double last_image_time = 0, last_left_time = 0;
bool init_pub = 0;

void stereo_callback(const sensor_msgs::ImageConstPtr &left_img,
                     const sensor_msgs::ImageConstPtr &right_img) {

}  // end of stereo_callback
...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg) {
  ...
  for (int i = 0; i < NUM_OF_CAM; i++) {
    ...
    if (i != 1 || !STEREO_TRACK) {
      /*std::cout << "img size before being read " << ptr->image.size()
                << std::endl;*/
      ...
      if (EQUALIZE) {
        ...
        /*std::cout << "img size after equalize " << trackerData[i].cur_img.size()
                  << std::endl;*/
      } else
  ...
  if (PUB_THIS_FRAME) {
    ...
    feature_points->header.frame_id = "map";  // change to map for r_l
